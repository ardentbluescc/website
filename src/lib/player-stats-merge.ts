// Shared merge logic for accumulating a single match's batting/bowling into a player's
// per-competition battingStats/bowlingStats rows. Used by both the manual scorecard-image
// import and the NV Play API sync, so the two stay consistent and de-duplicate the same way.

export function parseBalls(overs: number): number {
...
export function betterBB(a: string, b: string): string {
  if (!a || a === '-') return b || '-'
  if (!b || b === '-') return a
...
/** `${competition} · ${year}` — the grouping key battingStats/bowlingStats rows merge on. */
export function deriveFormat(competition: string | null | undefined, dateISO: string | null | undefined): string | null {
  if (!competition?.trim()) return null
  const year = dateISO ? new Date(dateISO).getFullYear() : null
  return year ? `${competition.trim()} · ${year}` : competition.trim()
}

// ── Batting merge ─────────────────────────────────────────────────────────────
// existing = current row for this format (null → first time)
// nm = the single-match batting data

export function mergeBatting(existing: any | null, nm: any, format: string, matchId: string | null) {
...
export function mergeBowling(existing: any | null, nm: any, format: string, matchId: string | null) {
...
export function buildMatchLogEntry(entryBatting: any | undefined, entryBowling: any | undefined, matchInfo: any) {
